...
export default function CompetitionsPage() {
  ...
  const [confed, setConfed] = useState<'all' | string>('all');
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error } = await supabase
        .schema('api') // important: our views live in the api schema
        .from(VIEW)
    ...
    // refresh every 90s (simple polling)
